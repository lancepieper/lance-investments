  ...
  return (
    <div>
      <svg viewBox={`0 0 ${w} ${h}`} className="w-full h-auto">
        ...
        {months.map((label, i) => (
          i % 2 === 0 && (
            <text key={i} x={toX(i)} y={h - 8} textAnchor="middle" fill="rgba(255,255,255,0.3)" fontSize={10} fontFamily="var(--font-geist-mono, monospace)">{label}</text>
          )
        ))}

        {/* Left axis: Gold */}
        <text x={m.l - 6} y={goldToY(lastGold) + 4} textAnchor="end" fill="#d4a84b" fontSize={10} fontFamily="var(--font-geist-mono, monospace)">${lastGold.toLocaleString()}</text>
        <text x={m.l - 6} y={goldToY(firstGold) + 4} textAnchor="end" fill="#d4a84b" fontSize={10} fontFamily="var(--font-geist-mono, monospace)">${firstGold.toLocaleString()}</text>
        <text x={m.l - 6} y={m.t - 4} textAnchor="end" fill="#d4a84b" fontSize={9} fontFamily="var(--font-geist-mono, monospace)">GOLD</text>

        {/* Right axis: Yield */}
        <text x={w - m.r + 6} y={yieldToY(lastYield) + 4} textAnchor="start" fill="#6b7280" fontSize={10} fontFamily="var(--font-geist-mono, monospace)">{lastYield.toFixed(2)}%</text>
        <text x={w - m.r + 6} y={yieldToY(firstYield) + 4} textAnchor="start" fill="#6b7280" fontSize={10} fontFamily="var(--font-geist-mono, monospace)">{firstYield.toFixed(2)}%</text>
        <text x={w - m.r + 6} y={m.t - 4} textAnchor="start" fill="#6b7280" fontSize={9} fontFamily="var(--font-geist-mono, monospace)">10Y</text>

        {/* Annotations */}
        <text x={toX(8)} y={goldToY(gold[8]) - 14} textAnchor="middle" fill="rgba(212,168,75,0.5)" fontSize={11} fontFamily="var(--font-geist-mono, monospace)">
          {goldChange} ↑
        </text>
        <text x={toX(8)} y={yieldToY(yields[8]) + 20} textAnchor="middle" fill="rgba(107,114,128,0.6)" fontSize={11} fontFamily="var(--font-geist-mono, monospace)">
          {yieldChange} →
        </text>
      </svg>
      <div className="flex justify-center gap-5 text-xs text-gray-500 mt-2">
